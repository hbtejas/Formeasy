...
export const PublicFormPage = () => {
  ...
  useEffect(() => {
    const run = async () => {
      if (!slug) return;
      const response = await publicApi.getFormBySlug(slug);
  ...
  const {
    ...
    watch,
    formState: { errors }
  } = useForm<Record<string, unknown>>({ resolver: zodResolver(schema) });
  ...
  if (success) {
    return (
      <div className="mx-auto mt-16 max-w-xl rounded-xl border bg-white p-8 text-center shadow-soft">
        <h2 className="mb-2 text-2xl font-semibold">Thanks for responding</h2>
    ...
    <div className="mx-auto max-w-2xl p-6">
      ...
      <form
        ...
        onSubmit={handleSubmit(async (values) => {
          try {
            const response = await responsesApi.submit(formConfig.id, values);
